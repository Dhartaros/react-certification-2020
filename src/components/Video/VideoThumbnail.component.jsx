...
export default function VideoThumbnail({id, title, description, thumbnail}) {
    ...
    function toggleFavorite() {
        ...
        try {
            ...
            if (videoIndex >= 0) {
                let filteredVideos = favoriteVideos.filter(video => video.id.videoId !== id);
                setFavoriteVideos(filteredVideos);
                console.log('%c[INFO] Video successfully removed from favorites.', LOG_STYLES.info);
            } else {
                let favorites = favoriteVideos.concat(videoToAdd);
                setFavoriteVideos(favorites);
                console.log('%c[INFO] Video successfully added to favorites.', LOG_STYLES.info);
                console.log(favoriteVideos);
            }
        } catch (error) {
            console.log('%c[WARN] There are no favorite videos added. Ignore the following error message.', LOG_STYLES.warn);
            ...
            <Card className={classes.root}>
                ...
                <CardActions>
                    ...
                    <Button size="small" color="secondary" onClick={toggleFavorite}>
                        {
                            favoriteVideos.findIndex(video => video.id.videoId === id) < 0 ? "Add to favorites" : "I LOVE THIS!"
                        }
                    </Button>
                    ) : (
