...
export default function VideoList() {
    ...
    useEffect(() => {
        let API_URL = `https://www.googleapis.com/youtube/v3/search?part=id,snippet&type=video&maxResults=21&q=${query}&key=${API_KEY}`;
    ...
    return (
        <Grid container spacing={0}>
            ...
            <Grid item container spacing={2} lg={8} md={10} sm={10} xs={10}>
            {
                videoList.map((video) => {
                    return (
                        <VideoThumbnail key={video.id.videoId} id={video.id.videoId}
